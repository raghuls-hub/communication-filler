import React, { useState, useEffect } from 'react';
import { registerUser } from '../../services/authService';
import { 
  getClasses, 
  getDepartments, 
  updateUserProfile,
  assignStudentToClass,
  assignTeacherToDepartment,
  assignClassTeacher
} from '../../services/dbService';

const AddUserForm = ({ onClose, initialUser = null }) => {
  const isEditMode = !!initialUser;

  const [formData, setFormData] = useState({
    email: '',
    password: '',
    displayName: '',
    role: 'student',
    department: '',
    classId: '',
    assignAsClassTeacherFor: '' // New Optional Field
  });

  const [departments, setDepartments] = useState([]);
  const [classes, setClasses] = useState([]);
  const [loading, setLoading] = useState(false);

  // Load Metadata
  useEffect(() => {
    const loadMetadata = async () => {
      try {
        const [d, c] = await Promise.all([getDepartments(), getClasses()]);
        setDepartments(d);
        setClasses(c);
      } catch (err) {
        console.error("Failed to load metadata", err);
      }
    };
    loadMetadata();

    if (isEditMode) {
      setFormData({
        email: initialUser.email || '',
        password: '***',
        displayName: initialUser.displayName || '',
        role: initialUser.role || 'student',
        department: initialUser.department || '',
        classId: initialUser.classId || '',
        assignAsClassTeacherFor: ''
      });
    }
  }, [initialUser, isEditMode]);

  const handleChange = (e) => {
    setFormData({ ...formData, [e.target.name]: e.target.value });
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);

    try {
      if (isEditMode) {
        // --- EDIT MODE ---
        const updates = {
          displayName: formData.displayName,
          role: formData.role,
          department: formData.department,
          classId: formData.classId
        };
        
        if (formData.role !== 'student') delete updates.classId;
        if (formData.role === 'admin') delete updates.department;

        await updateUserProfile(initialUser.id, updates);
        alert('User updated successfully!');
        if (onClose) onClose();
      
      } else {
        // --- CREATE MODE ---
        // Security Alert Removed as requested ("remove the security feature")
        // Note: Creating a user via Client SDK may still trigger a session change.

        const userData = {
          displayName: formData.displayName,
          role: formData.role,
          department: formData.department,
          classId: formData.classId
        };

        if (formData.role !== 'student') delete userData.classId;
        if (formData.role === 'admin') delete userData.department;

        // 1. Create Auth & User Doc
        const newUser = await registerUser(formData.email, formData.password, userData);

        // 2. Handle Relations (Arrays)
        if (formData.role === 'student' && formData.classId) {
          await assignStudentToClass(formData.classId, newUser.uid);
        }

        if (formData.role === 'teacher') {
          if (formData.department) {
            await assignTeacherToDepartment(formData.department, newUser.uid);
          }
          if (formData.assignAsClassTeacherFor) {
            await assignClassTeacher(formData.assignAsClassTeacherFor, newUser.uid);
          }
        }

        alert('User created successfully!');
        if (onClose) onClose();
      }
    } catch (error) {
      console.error('Operation failed:', error);
      if (error.code === 'auth/email-already-in-use') {
        alert('Error: Email already in use.');
      } else {
        alert('Error: ' + error.message);
      }
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="bg-white p-6 md:p-8 rounded-xl shadow-lg border border-slate-200 transition-all duration-300">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h3 className="text-xl font-bold text-slate-900">
            {isEditMode ? 'Edit User' : 'Add New User'}
          </h3>
          <p className="text-sm text-slate-500">
             {isEditMode ? 'Update user details.' : 'Create a new account.'}
          </p>
        </div>
        <button 
          onClick={onClose} 
          className="text-slate-400 hover:text-slate-600 p-2 hover:bg-slate-100 rounded-full transition-colors"
        >
          âœ•
        </button>
      </div>

      <form onSubmit={handleSubmit} className="space-y-5">
        
        {/* Email */}
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1.5">Email Address</label>
          <input
            type="email"
            name="email"
            required
            disabled={isEditMode}
            className="w-full border rounded-lg px-3 py-2.5 outline-none focus:ring-2 focus:ring-blue-500/20 "
            value={formData.email}
            onChange={handleChange}
          />
        </div>

        {/* Password */}
        {!isEditMode && (
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Password</label>
            <input
              type="password"
              name="password"
              required
              minLength={6}
              className="w-full border border-slate-300 rounded-lg px-3 py-2.5 outline-none focus:ring-2 focus:ring-blue-500/20"
              value={formData.password}
              onChange={handleChange}
            />
          </div>
        )}

        {/* Name */}
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1.5">Full Name</label>
          <input
            type="text"
            name="displayName"
            required
            className="w-full border border-slate-300 rounded-lg px-3 py-2.5 outline-none focus:ring-2 focus:ring-blue-500/20"
            value={formData.displayName}
            onChange={handleChange}
          />
        </div>

        {/* Role */}
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1.5">Role</label>
          <select
            name="role"
            className="w-full border border-slate-300 rounded-lg px-3 py-2.5 bg-white outline-none focus:ring-2 focus:ring-blue-500/20"
            value={formData.role}
            onChange={handleChange}
          >
            <option value="student">Student</option>
            <option value="teacher">Teacher</option>
            <option value="admin">Admin</option>
          </select>
        </div>

        {/* STUDENT FIELDS */}
        {formData.role === 'student' && (
          <div className="bg-blue-50/50 p-4 rounded-lg border border-blue-100/50 space-y-3">
             <div>
              <label className="block text-xs font-bold text-blue-800 uppercase tracking-wide mb-1">Assign Class</label>
              <select
                name="classId"
                required
                className="w-full border border-blue-200 rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500/20"
                value={formData.classId}
                onChange={handleChange}
              >
                <option value="">Select Class</option>
                {classes.map(c => (
                  <option key={c.id} value={c.name}>{c.name}</option>
                ))}
              </select>
              <p className="text-xs text-blue-600/70 mt-1">Student will be added to this Class roster.</p>
            </div>
          </div>
        )}

        {/* TEACHER FIELDS */}
        {formData.role === 'teacher' && (
           <div className="bg-indigo-50/50 p-4 rounded-lg border border-indigo-100/50 space-y-4">
            
            {/* Department */}
            <div>
              <label className="block text-xs font-bold text-indigo-800 uppercase tracking-wide mb-1">Department</label>
              <select
                  name="department"
                  required
                  className="w-full border border-indigo-200 rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500/20"
                  value={formData.department}
                  onChange={handleChange}
                >
                  <option value="">Select Department</option>
                  {departments.map(d => (
                    <option key={d.id} value={d.name}>{d.name}</option>
                  ))}
              </select>
              <p className="text-xs text-indigo-600/70 mt-1">Teacher will be added to this Dept roster.</p>
            </div>

            {/* Optional Class Teacher Assignment */}
            <div className="pt-2 border-t border-indigo-100/50">
              <label className="block text-xs font-bold text-indigo-800 uppercase tracking-wide mb-1">
                Assign as Class Teacher (Optional)
              </label>
              <select
                  name="assignAsClassTeacherFor"
                  className="w-full border border-indigo-200 rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500/20"
                  value={formData.assignAsClassTeacherFor}
                  onChange={handleChange}
                >
                  <option value="">-- None --</option>
                  {classes.map(c => (
                    <option key={c.id} value={c.name}>{c.name}</option>
                  ))}
              </select>
            </div>
          </div>
        )}

        <div className="flex gap-3 pt-4">
          <button
            type="submit"
            disabled={loading}
            className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 rounded-lg shadow-sm transition-all disabled:opacity-50"
          >
            {loading ? 'Processing...' : (isEditMode ? 'Update User' : 'Create User')}
          </button>
          <button
            type="button"
            onClick={onClose}
            className="px-6 py-2.5 border border-slate-300 text-slate-700 font-medium rounded-lg hover:bg-slate-50 transition-colors"
          >
            Cancel
          </button>
        </div>
      </form>
    </div>
  );
};

export default AddUserForm;
